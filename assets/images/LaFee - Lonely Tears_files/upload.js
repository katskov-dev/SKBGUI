...
onCheckComplete: function(iUpload, res) {
  ...
  var obj = {}, options = Upload.options[iUpload], data = res;
  if (options.signed) {
    obj = parseJSON(res || '{}');
  } else {
    res = res || '';
...
serverFail: function(iUpload, err_obj) {
  var obj = Upload.obj[iUpload], options = Upload.options[iUpload], vars = Upload.vars[iUpload];
  if (!obj) return;
  ...
  if (options.signed) {
    var parts = Upload.uploadUrls[iUpload].split('?'), tmp = q2ajx(parts[1]), q = extend({
      act: 'check_result',
...
serverSuccess: function(iUpload, err_obj) {
  var options = Upload.options[iUpload], vars = Upload.vars[iUpload];
...
onUploadComplete: function(info, result, extra_info) {
  var iUpload = info.ind !== undefined ? info.ind : info;
  var options = Upload.options[iUpload], obj;
...
onFileApiSend: function(iUpload, files, force) {
  ...
  if (!options.multi_progress) this.onUploadStart({ ind: iUpload, fileName: (files[0].fileName || files[0].name || '').replace(/[&<>"']/g, '') });
...
uploadPhotos: function(iUpload) {
  var files = [], flist = Upload.fileList[iUpload];
  for (var i in flist) {
    if (flist[i]) {
...
uploadFiles: function(iUpload, files, max_files) {
  ...
  var options = this.options[iUpload],
      vars = options.signed ? this.vars[iUpload] : extend(this.vars[iUpload], {ajx: 1}),
      totalSize = 0,
      totalCount = 0,
      loadedSize = 0,
      loadedCount = 0,
  ...
  for (var j = 0; j < max_files; j++) {
    ...
    if (options.multi_progress && !options.multi_sequence) {
      this.onUploadStart({ind: iUpload, fileName: fileName});
    }
...
terminateUpload: function(iUpload, name, el) {
  try {
    var vars = Upload.vars[iUpload],
        ...
        inQueue = false,
        info = options.multi_progress ? {ind: iUpload, fileName: name.replace(/[&<>"']/g, '')} : iUpload,
